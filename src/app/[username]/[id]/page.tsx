...
export default function EventoDetalhesPage() {
    ...
    try {
      ...
      if (breakdown.customerFinalPrice > 0) {
        ...
        const { url } = await createCheckoutSession({
          ...
          metadata: { registrationId: docRef.id, eventId, userId: user.uid, ticketCode }
        });
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col">
      <div className="max-w-6xl mx-auto px-4 pt-10 space-y-8 flex-1">
        <div className="relative h-[300px] md:h-[450px] w-full rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white">
          ...
          <div className="absolute bottom-10 left-10 text-white space-y-4">
             ...
             <h1 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter">{event.title}</h1>
             <div className="flex items-center gap-4 text-sm font-bold opacity-80"><MapPin className="w-4 h-4" /> {event.city}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
           <div className="lg:col-span-8 space-y-6">
              <Card className="border-none shadow-sm rounded-[2rem]">
                 <CardHeader><CardTitle className="flex items-center gap-2">Sobre o Evento</CardTitle></CardHeader>
                 <CardContent><p className="text-muted-foreground leading-relaxed whitespace-pre-line">{event.description}</p></CardContent>
              </Card>
           </div>
           
           <div className="lg:col-span-4">
              <Card className="border-none shadow-lg rounded-[2rem] border-t-8 border-secondary overflow-hidden">
                    ...
                    {saleStatus === 'open' && activeBatch ? (
                       <div className="space-y-6">
                                ...
                                {activeBatch.ticketTypes.map((type: any) => (
                                  ...
                                  >
                                     <div className="space-y-1">
                                        ...
                                        {type.requiresProof && <Badge variant="outline" className="text-[8px] font-black uppercase border-orange-200 text-orange-600">Requer Comprovação</Badge>}
                                     </div>
                          ...
                          <Button 
                            ...
                            className="w-full h-14 bg-secondary text-white font-black rounded-2xl shadow-xl uppercase italic"
                          >
                             Selecionar Ingresso
                          </Button>
                       </div>
                      ...
                      <div className="p-10 text-center space-y-2 bg-muted/20 rounded-2xl">
                         ...
                         <p className="font-black uppercase italic">{saleStatus === 'soldout' ? 'Esgotado' : saleStatus === 'pending' ? 'Em Breve' : 'Encerrado'}</p>
                      </div>
                    )}
                 </CardContent>
              </Card>
         ...
         <DialogContent className="rounded-[2.5rem] max-w-md p-0 overflow-hidden">
            <div className="p-8 space-y-6">
               ...
               {selectedTicketType?.requiresProof && (
                 <div className="p-4 rounded-xl bg-orange-50 border border-orange-100 flex gap-2">
                    <Info className="w-4 h-4 text-orange-600 shrink-0" />
                    <p className="text-[9px] text-orange-800 font-bold uppercase leading-relaxed">Este ingresso exige apresentação de documento comprobatório na portaria do evento.</p>
                 </div>
               )}
