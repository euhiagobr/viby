...
function handleStripeError(error: any): string {
  ...
  if (message.includes('rak_account_link_write') || message.includes('permissions')) {
    return "Sua chave de API do Stripe não tem permissão para criar links de onboarding. Edite a chave no dashboard do Stripe.";
  }
  ...
  try {
    ...
    if (accountId) {
      try {
        ...
        if (retrieveError.code === 'resource_missing' || retrieveError.message.includes('does not exist')) {
          accountId = null;
          await orgRef.update({
...
export async function retrieveStripeAccount(accountId: string, orgId?: string) {
  try {
    ...
    if (error.code === 'resource_missing' || error.message.includes('does not exist')) {
      ...
      return { 
        success: false, 
        error: 'A conta vinculada não existe mais no Stripe. O vínculo foi removido.' 
      };
    }
