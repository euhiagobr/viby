    ...
    <div className="min-h-screen bg-[#f8fafc] flex">
      <aside className="w-64 bg-primary text-white hidden lg:flex flex-col sticky top-0 h-screen">
        ...
        <nav className="flex-1 px-4 space-y-2">
          <p className="px-4 text-[10px] font-black uppercase text-white/40 tracking-widest mb-4">Menu Gestor</p>
          {navItems.map((item) => (
            <Link 
              key={item.url} 
              href={item.url}
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        <header className="h-16 border-b border-border bg-white flex items-center justify-between px-8 sticky top-0 z-50">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-sm uppercase tracking-widest">Painel Administrativo</h2>
          </div>
          <div className="flex items-center gap-4">
             <div className="flex flex-col items-end">
                <span className="text-xs font-bold">{user?.displayName || "Admin"}</span>
                <span className="text-[10px] font-bold text-muted-foreground uppercase">Modo Administrador</span>
             </div>
