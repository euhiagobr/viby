...
export default function AdminNotificacoesCreatorPage() {
  ...
  const handleSendNotification = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!db || !targetUser || !message.trim() || isSending) return;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <Card className="border-none shadow-sm rounded-[2rem] bg-white overflow-hidden">
               ...
               <CardContent className="p-8 space-y-8">
                  ...
                  <div className="space-y-3">
                     ...
                     <MentionTextarea 
                       placeholder="Escreva sua mensagem... Use @ para mencionar e ** para negrito. [instagramx=user] para links do IG." 
                       value={message}
                       onValueChange={setMessage}
                       className="min-h-[150px] p-6 rounded-3xl border-dashed border-secondary/30 focus-visible:ring-secondary/20"
                     />
                     <div className="flex flex-wrap gap-4 text-[9px] font-bold text-muted-foreground uppercase opacity-60 px-2">
                        <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> **bold**</span>
                        <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> @username</span>
                        <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> [instagramx=user]</span>
                     </div>
                  </div>

                  <Button 
                    onClick={handleSendNotification}
                    disabled={isSending || !targetUser || !message.trim()} 
                    className="w-full h-16 bg-primary text-white font-black rounded-2xl shadow-xl uppercase italic text-lg hover:scale-[1.01] transition-all"
                  >
         ...
         <div className="lg:col-span-5 space-y-6">
            ...
            <Card className="border-none shadow-xl rounded-[2.5rem] bg-white overflow-hidden">
               <div className="p-1 bg-secondary/10 flex items-center justify-center gap-2">
                  ...
                  <div className="flex items-start gap-4">
                     ...
                     <div className="flex-1 space-y-3">
                        <div className="flex justify-between items-start">
                           <div>
                              <h4 className="font-black text-sm uppercase italic text-primary leading-none">Viby Oficial</h4>
                        ...
                        <div className="bg-muted/30 p-5 rounded-3xl border border-dashed min-h-[100px]">
                           {message ? (
                             <RichText content={message} className="text-sm font-medium text-foreground/80" />
                           ) : (
                             <p className="text-xs italic text-muted-foreground/30 font-bold uppercase text-center mt-6">Sua mensagem aparecerá aqui...</p>
                           )}
                        </div>
                     </div>
                  </div>
            ...
            <div className="p-6 bg-secondary/5 rounded-3xl border border-secondary/10 flex items-start gap-4">
               ...
               <div className="space-y-1">
                  <h4 className="font-black uppercase text-[10px] tracking-widest text-secondary italic">Canal de Transmissão</h4>
                  <p className="text-[10px] text-muted-foreground leading-relaxed font-medium uppercase">
                    Estas notificações aparecem no sino de alertas dos usuários e geram um log imutável de auditoria administrativa.
                  </p>
               </div>
            </div>
         </div>
