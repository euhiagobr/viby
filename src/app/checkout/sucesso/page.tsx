...
function CheckoutSucessoContent() {
  ...
  React.useEffect(() => {
    if (!isInitialized || !sessionId || !user || isProcessing.current) return;
    ...
    const processSuccess = async () => {
      ...
      try {
        ...
        setError("Falha crítica ao validar sua transação no servidor.");
        setLoading(false);
      }
    };

    processSuccess();
  }, [sessionId, user, isInitialized, clearCart]);
          ...
          <CardContent className="p-10 space-y-6 text-center">
            ...
            <div className="flex flex-col gap-3">
               ...
               <Button variant="ghost" asChild className="font-bold text-xs uppercase text-muted-foreground">
                 <Link href="/dashboard/ingressos">Tentar ver meus ingressos</Link>
               </Button>
            </div>
          </CardContent>
        </Card>
