...
export default function OrganizationFinancePage() {
  ...
  const transactionsQuery = useMemoFirebase(() => {
    if (!db || !currentOrg) return null;
    return query(collection(db, 'organizations', currentOrg.id, 'transactions'), orderBy('createdAt', 'desc'), limit(100));
  }, [db, currentOrg?.id]);
  ...
  const handleTopUp = async () => {
    ...
    } catch (e) { toast({ variant: "destructive", title: "Erro no Checkout" }); }
           ...
           <Card className="border-none shadow-sm rounded-[2rem] bg-white p-8">
              <div className="flex flex-col md:flex-row gap-10">
                 <div className="flex-1 space-y-6">
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="flex gap-2">
                          ...
                          <Button onClick={handleTopUp} disabled={isTopUpLoading} className="h-12 bg-secondary text-white font-black px-8 rounded-xl uppercase italic">Recarregar</Button>
                       </div>
                    </div>
                 </div>
                 <div className="w-full md:w-1/3 p-6 bg-muted/30 rounded-3xl space-y-4">
                    ...
                    <p className="text-[10px] text-muted-foreground leading-relaxed uppercase">O saldo para anúncios é independente das vendas de ingressos e deve ser gerenciado por aqui.</p>
                 </div>
              </div>
           </Card>
        </TabsContent>
