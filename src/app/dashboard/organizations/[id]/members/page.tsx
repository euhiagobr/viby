...
export default function OrganizationMembersPage() {
  ...
  const handleInviteMember = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const identifier = (formData.get('identifier') as string).toLowerCase().trim();
    ...
    try {
      ...
      const qUsername = query(collection(db, 'usernames'), where('uid', '!=', ''), where('type', '==', 'user'));
      const usernamesSnap = await getDocs(collection(db, 'usernames'));
      const foundUsername = usernamesSnap.docs.find(d => d.id === identifier);
      
      if (foundUsername) {
        targetUid = foundUsername.data().uid;
      } else {
            ...
            <DialogContent className="max-w-md rounded-[2rem]">
              <form onSubmit={handleInviteMember} className="space-y-6">
                <DialogHeader>
                  ...
                  <DialogDescription>Insira o @username ou e-mail do usuário cadastrado na Viby.</DialogDescription>
                </DialogHeader>
                
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest opacity-60">Identificador</Label>
                    <Input name="identifier" placeholder="ex: @joaosilva ou joao@email.com" required className="rounded-xl h-11" />
                  </div>
           ...
           <TableBody>
              {loading ? (
                ...
                <TableRow key={member.id} className="hover:bg-muted/10 transition-colors">
                   <TableCell>
                      <div className="flex items-center gap-3">
                         <Avatar className="h-10 w-10 border border-muted">
                            ...
                            <AvatarFallback className="font-bold bg-secondary text-white">{member.profile?.displayName?.charAt(0) || 'U'}</AvatarFallback>
                         </Avatar>
                         <div className="flex flex-col">
                            <span className="font-bold text-sm">{member.profile?.displayName || 'Usuário'}</span>
                   ...
                   <TableCell>
                      <span className="text-[10px] font-bold text-muted-foreground uppercase">{new Date(member.createdAt?.seconds * 1000).toLocaleDateString('pt-BR')}</span>
                   </TableCell>
