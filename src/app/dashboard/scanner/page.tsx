  ...
  const validateTicket = async (input: string) => {
    ...
    try {
      ...
      if (snap.empty) {
        ...
        const ticketDate = safeParseDate(targetDoc.eventDate);
        const today = startOfToday();

        // VALIDAÇÃO DE DATA (REGRA CRÍTICA)
        const isCorrectDay = ticketDate && isSameDay(ticketDate, today);
        ...
        } else if (!isCorrectDay && targetDoc.productType === 'experience') {
           // Bloqueio específico para experiências em datas diferentes
           setTicketData({ ...targetDoc, invalidDate: true });
        } else {
    ...
    <div className="max-w-2xl mx-auto space-y-8 pb-20 pt-10 px-4 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-black italic tracking-tighter text-primary uppercase leading-none">Validar Acesso</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="hover:border-secondary transition-all cursor-pointer group rounded-[2.5rem] bg-white border-none shadow-sm" onClick={startCamera}>
            <CardContent className="p-10 flex flex-col items-center justify-center gap-6">
              <div className="p-8 bg-secondary/10 rounded-full group-hover:bg-secondary group-hover:text-white transition-all">
                <Camera className="w-12 h-12" />
              </div>
              <h3 className="font-black text-xl uppercase italic tracking-tighter text-center">Câmera</h3>
            </CardContent>
          </Card>
          <Card className="hover:border-secondary transition-all cursor-pointer group rounded-[2.5rem] bg-white border-none shadow-sm" onClick={() => setMode('manual')}>
            <CardContent className="p-10 flex flex-col items-center justify-center gap-6">
              <div className="p-8 bg-primary/5 rounded-full group-hover:bg-primary group-hover:text-white transition-all">
                <Keyboard className="w-12 h-12" />
              </div>
              <h3 className="font-black text-xl uppercase italic tracking-tighter text-center">Manual</h3>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      {ticketData && (
        <div className="animate-in zoom-in-95 duration-300">
           ...
           ) : ticketData.invalidDate ? (
             <Card className="border-none shadow-2xl rounded-[3rem] bg-red-600 text-white p-12 text-center space-y-6">
                <AlertCircle className="w-20 h-20 mx-auto opacity-40" />
                <h2 className="text-3xl font-black uppercase italic tracking-tighter leading-none">Data Incorreta</h2>
                <div className="space-y-1">
                   <p className="font-bold text-xs uppercase">Este ingresso é válido para outra data.</p>
                   <p className="text-lg font-black uppercase">{formatTicketDate(ticketData.eventDate).date}</p>
                </div>
                <div className="p-4 bg-black/20 rounded-2xl text-[10px] font-medium leading-relaxed uppercase">
                   O sistema bloqueia a entrada em dias diferentes do agendamento para garantir o controle de lotação.
                </div>
                <Button variant="outline" onClick={resetScanner} className="border-white text-white hover:bg-white hover:text-red-600 rounded-xl h-12 px-8 font-black uppercase">Voltar ao Início</Button>
             </Card>
           ) : (
