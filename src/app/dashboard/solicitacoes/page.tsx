  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground font-medium">Gerencie convites de acesso e acompanhe seus pedidos de saque.</p>
      </div>
      ...
      <Tabs defaultValue="invites" className="space-y-8">
        <TabsList className="bg-muted/50 p-1 rounded-xl h-12">
          ...
          <TabsTrigger value="payouts" className="rounded-lg px-8 font-bold gap-2">
             <SendHorizontal className="w-4 h-4" /> Seus Saques
          </TabsTrigger>
        </TabsList>
                      ...
                      <Card key={invite.id} className={cn("border-none shadow-sm rounded-[2rem] bg-white overflow-hidden group", isExpired && "opacity-50 grayscale")}>
                        ...
                        <CardContent className="p-6 space-y-6">
                          ...
                          <div className="flex gap-3">
                            <Button variant="outline" className="flex-1 rounded-xl h-11 text-destructive border-destructive font-bold text-[10px] uppercase" onClick={() => handleDeclineMember(invite)} disabled={actionLoadingId === invite.id}><X className="w-4 h-4 mr-2" /> {isExpired ? 'Remover' : 'Recusar'}</Button>
                            {!isExpired && <Button className="flex-1 rounded-xl font-black bg-green-600 text-white h-11 uppercase text-[10px] italic shadow-lg" onClick={() => handleAcceptMember(invite)} disabled={actionLoadingId === invite.id}>{actionLoadingId === invite.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4 mr-2" />} Aceitar</Button>}
                          </div>
                        </CardContent>
                      </Card>
                    );
                    ...
                    <Card key={saque.id} className="border-none shadow-sm rounded-[2rem] bg-white overflow-hidden">
                       <CardHeader className="bg-muted/30 pb-4">
                          <div className="flex justify-between items-start">
                             <div className={cn(
                                ...
                                saque.status === 'Concluído' ? "bg-green-500" : saque.status === 'Recusado' ? "bg-destructive" : "bg-orange-500"
                             )}>
                                {saque.status === 'Concluído' ? <CheckCircle2 className="w-5 h-5" /> : <Clock className="w-5 h-5" />}
                             </div>
                             <Badge variant="outline" className={cn(
                                "uppercase text-[9px] font-black h-5 px-2",
                                saque.status === 'Concluído' ? "border-green-200 text-green-600 bg-green-50" : "border-orange-200 text-orange-600 bg-orange-50"
                             )}>
                                {saque.status}
                             </Badge>
                          </div>
                       ...
                       <CardContent className="p-6 space-y-4">
                          <div className="space-y-1">
                             <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest">Protocolo #{saque.id.slice(-8)}</p>
                          ...
                          <div className="flex items-center justify-between text-[9px] font-bold uppercase opacity-60">
                             <span>Solicitado em</span>
                             <span>{saque.requestedAt?.toDate ? saque.requestedAt.toDate().toLocaleDateString('pt-BR') : new Date(saque.requestedAt).toLocaleDateString('pt-BR')}</span>
                          </div>
                          ...
                          {saque.status === 'Concluído' && saque.proofUrl && (
                             <Button variant="outline" size="sm" asChild className="w-full h-10 rounded-xl bg-green-50 border-green-200 text-green-600 font-bold uppercase text-[9px] gap-2">
                                <a href={saque.proofUrl} target="_blank" rel="noopener noreferrer">
