...
export function EventCard({ event, thematicTheme = 'default' }: EventCardProps) {
  ...
  }, [event.date, event.endDate, event.startTime, event.endTime, event.id, (event as any)._nextOccurrence]);
  ...
  const isEnded = React.useMemo(() => {
    ...
    return ended;
  }, [eventDates.end, mounted, event.isRecurring, event.recurringEndDate, event.id]);
