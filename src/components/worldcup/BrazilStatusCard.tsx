...
export function BrazilStatusCard({ standings, nextMatch, loading }: BrazilStatusCardProps) {
  ...
  const isEliminated = React.useMemo(() => {
    if (!nextMatch && brazilStats?.entry.position && brazilStats.entry.position > 2) return true;
    return false;
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
          <div className="space-y-4">
            <p className="text-[10px] font-black uppercase opacity-40 tracking-widest flex items-center gap-2">
              ...
              {nextMatch?.stage === 'GROUP_STAGE' || !nextMatch ? `Grupo ${brazilStats?.group || '---'}` : 'Status Atual'}
            </p>
            <div className="space-y-2">
              {loading ? <Skeleton className="h-10 w-32 bg-white/10 rounded-lg" /> : (
                <div className="flex justify-between items-end">
                  <span className="text-3xl font-black">{nextMatch ? STAGE_LABELS[nextMatch.stage] : 'Encerrado'}</span>
                </div>
              )}
              {!isEliminated && (
          ...
          <div className="space-y-4">
            <p className="text-[10px] font-black uppercase opacity-40 tracking-widest flex items-center gap-2">
              {nextMatch?.status === 'IN_PLAY' ? <Zap className="w-3.5 h-3.5 text-red-500 animate-pulse fill-current" /> : <Clock className="w-3.5 h-3.5" />}
              {nextMatch?.status === 'IN_PLAY' ? 'Jogo em Andamento' : 'Próximo Jogo'}
            </p>
            {loading ? <div className="space-y-2"><Skeleton className="h-4 w-40 bg-white/10" /><Skeleton className="h-3 w-32 bg-white/10" /></div> : nextMatch ? (
              <div className="space-y-1">
                <p className="font-black text-sm uppercase italic text-[#ffdf00] truncate">
                  vs {nextMatch.homeTeam?.id === BRAZIL_ID ? nextMatch.awayTeam?.name : nextMatch.homeTeam?.name}
                </p>
            ...
            ) : <p className="text-xs opacity-60 uppercase font-black">{isEliminated ? 'Eliminado da competição' : 'Aguardando definição'}</p>}
          </div>
