 ...
 * Busca imagens reais em repositórios públicos e salva no Firebase Storage.
 */
...
export async function processCityCoverGeneration(params: {
  ...
  try {
    console.log(`[CITY COVER SERVICE] Buscando imagem real para: ${city}`);
    
    // 1. Termos de busca para imagens reais
    ...
    if (buffer.length < 5000) throw new Error("Imagem inválida.");
